...
# Generate csv file
python "${pkgdir}/utils/gen_jobs_time_expers_data.py" "${tmpfile}" > "${resultsdir}/jobs_time_expers_data_unsorted.csv"

# Sort csv file
(head -n 1 "${resultsdir}/jobs_time_expers_data_unsorted.csv" && tail -n +2 "${resultsdir}/jobs_time_expers_data_unsorted.csv" | sort -t, -k1,1n) > "${resultsdir}/jobs_time_expers_data.csv"
