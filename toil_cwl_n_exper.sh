...
pushd "${resultsdir}"
conda activate toil
/bin/time -f "%e %M" -o "${resultsdir}/time_command_$n" toil-cwl-runner --jobStore myStore --batchSystem slurm --outdir toil_out --workDir toil_executions --maxCores $num_procs --disableCaching "${infdir}/${expertype}.cwl" "${resultsdir}/input.yml" > "${resultsdir}/${toolname}.log" 2>&1
conda deactivate
popd
